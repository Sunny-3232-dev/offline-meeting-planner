...
■ レイアウト構成：
・参考画像と同じ要素配置を再現すること。
　（タイトル位置・キャッチコピー位置・アイコン位置・キャラクター位置など）
・余白・フレーム・帯などの装飾構造も参考画像に合わせる。

■ アイコン・図解：
・参考画像のアイコンや図解はそのまま使わず、新しいサービス内容に合ったものに自動で差し替えること。
　（例：スプレッドシート→表アイコン、ITサポート→PCやヘルプデスクアイコン）
...
※ このプロンプトをGeminiに貼り付ける際に、参考にしたいサムネイル画像を一緒に添付してください。`,
  };
